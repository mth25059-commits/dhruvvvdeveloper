        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="relative grid h-9 w-9 place-items-center rounded-full border border-white/10 bg-white/5 backdrop-blur-md transition-all hover:border-[rgb(var(--accent-violet)/0.5)] md:hidden"
            >
              <AnimatePresence mode="wait" initial={false}>
                {mobileOpen ? (
                  <motion.span
      ...
      <AnimatePresence>
        {mobileOpen && (
          ...
          >
            {/* Backdrop */}
            <div
              className="absolute inset-0 bg-[rgb(var(--bg-base)/0.95)] backdrop-blur-xl"
            ...
            >
              {NAV_LINKS.map((link, i) => {
                ...
                return (
                  <motion.a
                    ...
                    className={cn(
                      "text-2xl font-display font-semibold tracking-tight transition-colors",
                      isActive
                        ? "gradient-text"
                        : "text-[rgb(var(--text-muted))] hover:text-[rgb(var(--text-primary))]",
                    )}
              ...
              <motion.a
                ...
                initial={{ y: 20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                transition={{ delay: 0.15 + NAV_LINKS.length * 0.05, duration: 0.4 }}
                className="mt-4 rounded-full bg-[rgb(var(--accent-violet))] px-8 py-3 text-base font-medium text-white shadow-[0_0_30px_rgb(var(--accent-violet)/0.5)] transition-all hover:shadow-[0_0_50px_rgb(var(--accent-violet)/0.9)]"
              >
                Let&apos;s talk →
              </motion.a>
            </motion.nav>
